...
* For a given query, the engine provides three `interactive_query_mode` values to control the “solution output mode”:
  ...
  * See [interactive_query_mode](./config#7-interactive_query_mode) for detailed configuration.
...
* About inference paths:
  ...
  * It is recommended to enable `Args.run.trace=True`, and analyze rule triggering and the inference process with trace / logs / metrics (see [trace](./config#4-trace)).
...
3. **How do I integrate it into my own project?**
   ...
   * MCP (WIP)
   * Customize fact and rule selection modules for task scenarios; see [custom modules](../custom_module)
